...
namespace data
{
	...
	*/
	...
	template<uint M, uint N, typename Container>
	Matrix<M, N, Container>::Matrix(std::initializer_list<Scalar> values) : Matrix()
	{
		int i = 0;
	...
	template<uint M, uint N, typename Container>
	inline
		Scalar Matrix<M, N, Container>::operator()(const uint i, const uint j) const
	{
	...
	template<uint M, uint N, typename Container>
	inline
		Scalar& Matrix<M, N, Container>::operator()(const uint i, const uint j)
	{
		return this->values[getIndex(i, j)];
	}
	...
	*/
	template<uint M, uint N, typename Container>
	Matrix<M, N, Container> operator*(const Matrix<M, N, Container>& m, const Scalar x)
	{
		Matrix<M, N, Container> result;
	...
	*/
	template<uint M, uint N, typename Container>
	Matrix<M, N, Container> operator*(const Scalar x, const Matrix<M, N, Container>& m)
	{
		return m * x;
	}
...
namespace std
{
	...
	template <uint M, uint N, typename Container>
	inline std::ostream& operator<<(std::ostream &os, const Matrix<M, N, Container>& mat)
	{
		for (uint i = 0; i < M; i++)
		{
			for (uint j = 0; j < N; j++)
